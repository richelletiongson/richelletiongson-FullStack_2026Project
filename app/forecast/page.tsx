...
export default async function ForecastPage({ searchParams }: Props) {
  ...
  const prediction =
    sign && category && !Number.isNaN(monthId)
      ? getPrediction(sign, monthId, category)
      : null;
        ...
        {prediction ? (
          <>
            <p className="text-sm text-[var(--text-muted)]">
              {sign} · {category} · {monthName} 2026
            </p>
            <h1 className="font-display text-2xl font-bold text-[var(--text)] mt-1">
              Your forecast
            </h1>
            <div className="mt-6 rounded-[var(--radius)] border border-[var(--surface-hover)] bg-[var(--surface)] p-6">
