...
type Props = {
  // Label shown above the animation, e.g. "Taurus · Money · April 2026"
  label: string;
  // The actual horoscope text pulled from the database
  prediction: string;
};
 ...
 * CookieForecast
 *
 * Renders the interactive "click the cookie" animation and overlays the
 * forecast text on top of the revealed cookie background (asset-1.png).
 *
 * The animation timeline is:
 * 1. User clicks the cookie → start shake.
 * 2. After 1.6s → play blinding light overlay.
 * 3. After 1.9s → fade in the large cookie and prediction text.
 */
export default function CookieForecast({ label, prediction }: Props) {
  // Whether the user has triggered the animation (prevents replay).
  const [hasPlayed, setHasPlayed] = useState(false);
  // Controls the wobble / shake animation on the small cookie.
  const [shake, setShake] = useState(false);
  // Controls the full-screen radial flash overlay.
  const [flash, setFlash] = useState(false);
  // Controls the reveal of asset-1 + forecast text.
  const [reveal, setReveal] = useState(false);

  useEffect(() => {
    if (!hasPlayed) return;

    // Immediately start the shake.
    setShake(true);

    // Slight delay before the flash so the wobble is visible.
    const flashTimer = setTimeout(() => {
      setFlash(true);
    }, 1600);

    // A bit after the flash begins, fade in the revealed cookie + text.
  ...
  const handleClick = () => {
    // Only allow the animation to run once per page view.
    if (hasPlayed) return;
    setHasPlayed(true);
  };
    ...
    <section className="cookie-forecast-wrapper">
      ...
      <div className="cookie-viewport">
        <div className="cookie-scene">
          ...
          {/* Front cookie (asset-2.png) that the user clicks */}
          <img
          ...
          {/* Full-screen blinding light that briefly covers the page */}
          <div
          ...
          {/* Revealed cookie background (asset-1.png) with forecast text */}
          <div
